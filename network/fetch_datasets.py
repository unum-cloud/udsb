...
def download_datasets():
    for name,url in dataset_urls.items():
        path = dataset_save_path + name + '.' + os.path.basename(url).split(".")[-1]
        u = urlopen(url)
...
def datasets_to_edges():
    ...
    with zipfile.ZipFile(path, 'r') as zip_ref:
        dataset_paths[names[0]] = Edges(zip_ref.open("large_twitch_edges.csv", "r"))
    
    ## citation_patent
    path = dataset_urls[names[1]]
    dataset_paths[names[1]] = Edges(path, compression="gzip", sep='\t', header=3)

    ## live_j
    path = dataset_urls[names[2]]
    dataset_paths[names[2]] = Edges(path, compression="gzip", sep='\t', header=3)

    ## gplus
    path = dataset_urls[names[3]]
    dataset_paths[names[3]] = Edges(path, compression="gzip", sep=' ', header=0)
    
    ## stack
    path = dataset_urls[names[4]]
    dataset_paths[names[4]] = Edges(path, compression="gzip", sep='\t', header=3)
    
    ## orkut
    path = dataset_urls[names[5]]
    dataset_paths[names[5]] = Edges(path, compression="gzip", sep='\t', header=3)

    ## neuron
    path = dataset_urls[names[6]]
    dataset_paths[names[6]] = Edges(path, compression="gzip", sep='\t', header=3)
...
class Edges:
    ...
    def __init__(self, csv, sep=',', compression=None, header=0, skiprows=None) -> None:
        self.df = pd.read_csv(csv, sep=sep, compression=compression, header=header, skiprows=skiprows)
